...
public class MapGenerator : Module
{
    ...
    [HideInInspector]
    public List<Coord> spawnPoints = new List<Coord>();
    ...
    public void GenerateMap()
    {
        ...
        SetSpawnPoints(0, map.mapSize.x - 1, 0, map.mapSize.y - 1, tileSize);
    ...
    private void SetSpawnPoints(int initialX, int finalX, int initialY, int finalY, int step, float overlapBoxDimension = 2f)
    {
        spawnPoints.Clear();
        for (int x = initialX; x <= finalX; x += step)
    ...
    public Coord GetTileFromPosition(Vector3 position)
    {
        ...
        //int x = Mathf.RoundToInt(position.x + map.mapSize.x / 2f - 0.5f - transform.position.x);
        //int y = Mathf.RoundToInt(position.z + map.mapSize.y / 2f - 0.5f - transform.position.z);

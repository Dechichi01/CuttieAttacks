...
public class Spawner : MonoBehaviour
{
    ...
    void Start()
    {
        foreach (SpawnRect rect in spawnRects)  rect.SetOwner(transform);
    ...
    void SpawnPatrolEnemes()
    {
        ...
        */
        ...
        for (int i = 0; i < spawnRects.Length; i++)
        {
            for (int j = 0; j < enemisCount[i]; j++)
            {
                Vector3 spawnPos = mapGen.GetSpawnPositionInRect(spawnRects[i], 1) + Vector3.up;
                ...
                spawnedEnemy.isPatrolEnemy = true;
                enemiesRemainingToSpawn--;
            }
        }
    }
